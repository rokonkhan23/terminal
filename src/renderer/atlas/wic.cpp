...
void WIC::LoadTextureFromFile(ID3D11Device* device, const wchar_t* fileName, ID3D11Texture2D** out_texture, ID3D11ShaderResourceView** out_textureView)
{
    ...
    THROW_IF_FAILED(converter->CopyPixels(nullptr, static_cast<UINT>(stride), static_cast<UINT>(bytes), buffer.data()));
    ...
    const D3D11_SUBRESOURCE_DATA initData = {
        .pSysMem = buffer.data(),
        .SysMemPitch = static_cast<UINT>(stride),
        .SysMemSlicePitch = static_cast<UINT>(bytes),
    };
    wil::com_ptr<ID3D11Texture2D> texture;
